...
import math
import nltk
from inverted_index import build_dataset,build_inverted_index,trim_query,modified_query,generate_vocabulary,get_query_rel_docs
...
def rsj(term,R):
	...
	score=float(math.log(((term_rel_docs+0.5)*(N-R_-term_docs+term_rel_docs+0.5))/((term_docs-term_rel_docs+0.5)*(R_-term_rel_docs+0.5)))/math.log(2))
	return score
...
def weight(term,document):
	...
	term_docs=len(inverted_index[term])
	score=(tf/(k*((1-b)+b*(doc_length/avg_len_docs))+tf))*float(math.log((total_docs-term_docs+0.5)/term_docs+0.5)/math.log(2))
	return score
...
print("Original Preprocesses query : ",query)
query=modified_query(query,len(documents),inverted_index)
